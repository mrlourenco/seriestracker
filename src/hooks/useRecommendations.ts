...
async function callGemini(series: Array<{ title: string; status: string; rating: number | null }>) {
  const apiKey = import.meta.env.VITE_GEMINI_API_KEY as string | undefined
  if (!apiKey) throw new Error('Adiciona VITE_GEMINI_API_KEY ao ficheiro .env.local')
...
[{"title":"título original em inglês","reason":"motivo curto em português, 1 frase"}]`
  ...
  return JSON.parse(cleaned) as Array<{ title: string; reason: string }>
}
